...
export function useKioskRotation() {
  useEffect(() => {
    ...
    // Vanilla rotation — every `intervalMs`, advance to the next source.
    // Earlier versions had smart-pin-on-queue logic that flipped the TV
    // to the agent-states board whenever a call hit the queue. Now the
    // sales leaderboards surface the queue depth themselves via a
    // top-of-board banner (see <QueueBanner>), so the rotation stays
    // a simple cycle and the floor manager doesn't lose context every
